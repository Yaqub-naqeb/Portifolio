...
{/* img */}
<div>
<img src={info.img} className='lg:w-[35rem] rounded-lg md:w-[35rem] ' alt="project img" />
</div>
{/* description */}
...
<p className=' lg:w-[30vw] md:w-[60vw]  lg:text-left text-left md:text-center'>
  {/* lg:px-0 md:px-0 md:text-left text-center lg:pl-0 md:pl-0  pl-[1rem] */}
  {info.description.map((row,index)=>(<p key={index}>{row.p}</p>))}</p>


{/* tools */}
{/*  ${id==3?'lg:translate-x-[3.8rem] md:translate-x-[2rem]  ':''} */}
<div className={`flex lg:translate-x-[0rem]  m-[2rem] lg:gap-0  md:gap-2  gap-2 text-center text-[#306CC4] font-medium text-[1rem]`}>
...
<div className={`mt-5   flex gap-8 align-middle justify-center text-[1.2rem]`}>
 ...
 <a target={'_blank'} rel="noreferrer" href={`${info.demo}`} alt={'Demo'} title='Demo' className={` ${!info.demo ?'hidden ':'flex'} border-2 rounded-full p-2 border-[#ADD6E8] hover:bg-[#ADD6E8] hover:text-[#4d4d4d]  transition-all duration-500  ${mode?'text-[#4d4d4d]':'text-[#ffffff]'}`}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  ...
  {/* code braket */}
  <a target={'_blank'} href={info.url} rel="noreferrer" alt='Source code' title='Source code' className={` ${!info.code?'hidden ':'flex'}         border-2  rounded-full p-2 border-[#ADD6E8] hover:bg-[#ADD6E8] hover:text-[#4d4d4d] transition-all duration-500  ${mode?'text-[#4d4d4d]':'text-[#ffffff]'}`}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
